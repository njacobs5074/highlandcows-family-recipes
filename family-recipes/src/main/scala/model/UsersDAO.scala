...
class UsersDAO(database: Database) {
  ...
  def insert(user: User): User = database.ctx.transaction {
  ...
  def find(id: Int): Option[User] = {
    val action = quote(for {
      ...
      familyRecipeInstance <- database.schema.familyRecipeInstances.leftJoin(
        familyRecipeInstance => familyRecipeInstance.id == user.familyRecipeInstanceId
      )
  ...
  def update(user: User): User = {
    ...
    database.ctx.transaction {
      database.ctx.run(action) match {
        case 1     => database.ctx.run(database.schema.users.filter(_.id == lift(user.id))).head
        case 0 | _ => throw new RuntimeException(s"Failed to update $user")
      }
    }
  }
  ...
  def upsert(user: User): User = database.ctx.transaction {
    find(user.id) match {
